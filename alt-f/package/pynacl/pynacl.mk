...
#PYNACL_VERSION = 1.2.1
PYNACL_VERSION = 1.3.0
#PYNACL_SOURCE = $(PYNACL_VERSION).tar.gz
#PYNACL_SITE = https://github.com/pyca/pynacl/archive
PYNACL_SITE = https://pypi.org/packages/source/p/pynacl
PYNACL_SOURCE = PyNaCl-$(PYNACL_VERSION).tar.gz
...
$(PYNACL_DIR)/.unpacked: $(DL_DIR)/$(PYNACL_SOURCE)
	mkdir -p $(PYNACL_DIR)
	$(PYNACL_CAT) $(DL_DIR)/$(PYNACL_SOURCE) | tar $(TAR_STRIP_COMPONENTS)=1 -C $(PYNACL_DIR) $(TAR_OPTIONS) -
	touch $@
...
$(TARGET_DIR)/$(PYNACL_TARGET_BINARY): $(PYNACL_DIR)/.build
	...
	touch $(TARGET_DIR)/$(PYNACL_TARGET_BINARY)

pynacl: libsodium python cffi $(TARGET_DIR)/$(PYNACL_TARGET_BINARY)

pynacl-unpack: $(PYNACL_DIR)/.unpacked

pynacl-build: libsodium python cffi $(PYNACL_DIR)/.build

pynacl-install: libsodium python cffi $(TARGET_DIR)/$(PYNACL_TARGET_BINARY)
