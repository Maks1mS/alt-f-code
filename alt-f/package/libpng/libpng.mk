...
LIBPNG_SITE = $(BR2_SOURCEFORGE_MIRROR)/project/libpng/libpng16/$(LIBPNG_VERSION)
...
$(LIBPNG_HOOK_POST_INSTALL):
	$(SED) "s,^prefix=.*,prefix=\'$(STAGING_DIR)/usr\',g" \
		...
		-e "s,^includedir=.*,includedir=\'$(STAGING_DIR)/usr/include/libpng16\',g" \
		-e "s,^libdir=.*,libdir=\'$(STAGING_DIR)/usr/lib\',g" \
		$(STAGING_DIR)/usr/bin/libpng16-config
	rm -f $(TARGET_DIR)/usr/bin/libpng16-config \
		$(TARGET_DIR)/usr/bin/libpng-config \
		$(TARGET_DIR)/usr/bin/pngfix \
		$(TARGET_DIR)/usr/bin/png-fix-itxt
	touch $@
